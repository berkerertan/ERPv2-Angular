...
<title>Teklif ${esc(q.quoteNumber)}</title>
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .hdr{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:14px;border-bottom:3px solid #ea580c;margin-bottom:18px}
  .co-name{font-size:18pt;font-weight:900;color:#ea580c;letter-spacing:-0.5px}
  ...
  .qt-num{font-size:11pt;color:#ea580c;font-weight:700;text-align:right;margin-top:3px}
  ...
  .cust{background:#f7f8ff;border-left:4px solid #ea580c;padding:11px 15px;margin-bottom:18px;border-radius:0 6px 6px 0}
  ...
  .tot-grand{background:#ea580c;color:#fff;padding:10px 13px;border-radius:6px;margin-top:8px;font-size:14pt;font-weight:900;border:none}
...
<div class="hdr">
  <div>
<<<<<<< HEAD
    <div class="co-name">${esc(this.companyName)}</div>
    <div class="co-info">${esc(this.companyAddress)}<br>Tel: ${esc(this.companyPhone)} &nbsp;·&nbsp; E-posta: ${esc(this.companyEmail)}<br>Vergi No: ${esc(this.companyTaxNo)}</div>
  </div>
  <div>
    <div class="qt-label">TEKLİF</div>
    <div class="qt-num">${esc(q.quoteNumber)}</div>
=======
...
>>>>>>> 1a33f18e17d435247e7de2a69139e0d6bad344be
  </div>
</div>

<div class="meta">
<<<<<<< HEAD
  <div class="mbox"><label>Teklif Tarihi</label><span>${esc(q.date)}</span></div>
  <div class="mbox"><label>Geçerlilik Tarihi</label><span>${esc(q.validUntil)}</span></div>
  <div class="mbox"><label>KDV Oranı</label><span>%${q.taxPercent}</span></div>
  <div class="mbox"><label>Durum</label><span>${esc(this.statusLabel(q.status))}</span></div>
=======
...
<div class="cust">
<<<<<<< HEAD
  <div class="cust-name">${esc(q.customerName)}</div>
  <div class="cust-info">${[q.customerPhone ? 'Tel: ' + esc(q.customerPhone) : '', q.customerEmail ? 'E-posta: ' + esc(q.customerEmail) : ''].filter(Boolean).join(' &nbsp;·&nbsp; ')}</div>
=======
...
<div class="sigs">
  <div class="sig">Teklifi Hazırlayan<br><br><br>${esc(this.companyName)}</div>
  <div class="sig">Müşteri Onayı<br><br><br>${esc(q.customerName)}</div>
</div>
<div class="foot">Bu teklif ${esc(q.validUntil)} tarihine kadar geçerlidir. &nbsp;·&nbsp; ${esc(this.companyName)} &nbsp;·&nbsp; ${esc(this.companyEmail)}</div>
    ...
    private buildReceiptHtml(q: Quote): string {
        ...
        const linesHtml = q.lines.map(l => {
            ...
            return `
            <div class="item-name">${esc(l.productName)}${disc}</div>
            <div class="item-calc">
                <span>${l.quantity} ${esc(l.unit)} x ${fc2(l.unitPrice)}</span>
        ...
        return `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8">
<meta http-equiv="Content-Security-Policy" content="script-src 'none'">
<title>Fiş ${esc(q.quoteNumber)}</title>
<style>
...
<div class="c b lg">${esc(this.companyName)}</div>
<div class="c" style="font-size:8pt">${esc(this.companyPhone)}</div>
<div class="c" style="font-size:8pt">${esc(this.companyAddress)}</div>
<div class="dline"></div>
<div class="c b" style="font-size:12pt;letter-spacing:4px">TEKLİF</div>
<div class="sline"></div>
<div class="row"><span>No :</span><span>${esc(q.quoteNumber)}</span></div>
<div class="row"><span>Tarih :</span><span>${esc(q.date)}</span></div>
<div class="row"><span>Gecerlilik :</span><span>${esc(q.validUntil)}</span></div>
<div class="dline"></div>
<div class="b">Musteri: ${esc(q.customerName)}</div>
${q.customerPhone ? `<div style="font-size:8.5pt">Tel: ${esc(q.customerPhone)}</div>` : ''}
<div class="dline"></div>
${linesHtml}
<div class="dline"></div>
...
<div class="dline"></div>
${q.notes ? `<div style="font-size:8.5pt;margin-top:3px"><b>Not:</b> ${esc(q.notes)}</div><div class="sline"></div>` : ''}
<div class="ft">Bu teklif ${esc(q.validUntil)} tarihine kadar gecerlidir.</div>
